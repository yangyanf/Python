...
#设计网 http://www.cndesign.com/competition
#进行中 http://www.cndesign.com/competition/underway/list_2.html
def get_conn():
    '''建立数据库连接'''
...
def get_list_row(html):
    ...
    act_list = soup.find('ul', attrs={'class': 'c-l_ul'})
    #获取对应数据
    act_title = []#标题
    act_href = []#文章链接地址
    act_img = []#封面图
    ...
    for i in act_list.find_all('li'):
        title = i.find('h3', attrs={'class': 'saishi'}).get("title")
        href = i.find('h3', attrs={'class': 'saishi'}).find('a').get("href")
        cover_img = i.find('img').get("data-original")
        time = i.find('div', attrs={'class': 'competition_time'}).find('span').get_text()
        ...
        act_href.append("http://www.cndesign.com"+href)
...
def main():
    ...
    # exit()
    name_list = [] #各个活动链接地址
    for i in range(1, 3):#获取1-18 页数据
        url = "http://www.cndesign.com/competition/underway/list_"+str(i)+".html"
        html = get_html(url)
